...
public class MemberInfoGetRes {
	...
	String email;
	@ApiModelProperty(name = "회원 권한(Auth)")
	String role;
	@ApiModelProperty(name = "탈퇴한 회원인지 표시")
	boolean is_quit = false;
	@ApiModelProperty(name = "회원 닉네임")
	String nickname;
	@ApiModelProperty(name = "회원이 탈퇴한 날짜")
	String quit_at;
	@ApiModelProperty(name = "회원의 백준 온라인 저지 ID")
	String boj_id;
	@ApiModelProperty(name = "회원의 백준 온라인 저지 등급")
	String boj_rank;
	@ApiModelProperty(name = "회원의 프로필 사진명")
	String profile_name;
	@ApiModelProperty(name = "회원의 프로필 사진 경로")
	String profile_path;
	@ApiModelProperty(name = "신고 누적 회수")
	int report;
